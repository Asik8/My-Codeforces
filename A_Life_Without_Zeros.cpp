...
void asikM(){
    ...
    cin>>a>>b;
    ll s=a+b;
    string l=to_string(a),r=to_string(b),nl,nr,an=to_string(s),ans;
    fl(i,0,sz(l),1) if(l[i]!='0') nl+=l[i];
    fl(i,0,sz(r),1) if(r[i]!='0') nr+=r[i];
    fl(i,0,sz(an),1) if(an[i]!='0') ans+=an[i];
    // cout<<nl<<" "<<nr<<el
    // co((stoll(nl)+stoll(nr)))
    if(stoll(ans)!=(stoll(nl)+stoll(nr))) pn
    py
}   
